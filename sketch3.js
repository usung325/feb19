...
let canvasY = 100;
let numTile = 80; //180
let rows = canvasX; //numTile
let cols = canvasY; //numTile
let new2dArr = [];


function setup() {
  createCanvas(1920, 1920); //canvasX, canvasX
  console.log(arr);
  //initiate the array
  make2dArr(rows, cols);
}
  ...
  for (i = -1; i < 2; i++){
    for (j = -1; j < 2; j++){
      ...
      if (tempX < 0 || tempY < 0 || tempX > rows-1 || tempY > cols-1 || (tempX == x && tempY == y)){
        ...
        sum = sum + prevArr[tempX][tempY]; // REVERSE TEMPX TEMPY FOR MIRROR
